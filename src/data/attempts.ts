...
export type AttemptKind = "standard" | "tiny";

export function isTinyAttempt(attempt: Pick<Attempt, "kind">) {
  return attempt.kind === "tiny";
}
...
export interface Attempt {
  ...
  icon?: string;
  kind?: AttemptKind;
  /** Free-form notes — what you're trying and why. Tiny experiments use this instead of tasks. */
  description?: string;
  tasks: AttemptTask[];
 ...
 */
export function deadlineMissDays(
  attempt: Attempt,
  asOf: Date = new Date(),
): number | null {
  ...
  return Math.round(
    (startOfDay(finished).getTime() - startOfDay(attempt.deadline).getTime()) /
      86_400_000,
  );
}
